...
function completedJs() {
  ...
  var mask = new Vue({
    ...
    watch: {
      ...
      isNext: function(val) {
        mask.nextPrevSwitch(val, ".btn-next");
      },
      isPrev: function(val) {
        mask.nextPrevSwitch(val, ".btn-prev");
      }
    },
    methods: {
      nextPrevSwitch: function(flag, selection) {
        if (flag) {
          $(selection)
            .removeClass("fadeOut")
            .addClass("fadeIn");
      ...
      show: function() {
        mask.src = _.find(mask.intros, function(val) {
          return val.index == mask.index;
        }).img;
      },
      close: function() {
      ...
      change(callback) {
        $(".mask").addClass("fadeOutRight");
        _.delay(function() {
          callback.call(this);
          $(".mask").removeClass("fadeOutRight");
        }, 800);
      }
    }
  });
